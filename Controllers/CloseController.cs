...
public class CloseController : Controller
{
    ...
    public CloseController(CloseService close, InvestmentService investments, InvestmentPartnerService contracts, IFcmsLogService log)
    {
        _close = close;
    ...
    {
        var (ok, error, req) = await _close.RequestCloseAsync(investmentId, Guid.Empty, notes, ct);
        if (!ok)
        {
        ...
        await _log.LogAsync("investpro.close.request", nameof(CloseRequest), req!.Id.ToString(),
            value: new { investmentId, req.Id }, module: InvestProModule.ModuleIdValue, ct: ct);
    ...
    {
        var (ok, error, finalStatus, snapshotId) = await _close.DecideAsync(requestId, form.PartnerId, form.Decision, form.Comment, null, ct);
        if (!ok)
        {
        ...
        await _log.LogAsync($"investpro.close.{form.Decision.ToString().ToLower()}", nameof(CloseRequest), requestId.ToString(),
            value: new { investmentId, requestId, form.PartnerId, form.Decision, finalStatus = finalStatus.ToString(), snapshotId },
